...
import sqlsolver.liastar.LiaSolverStatus;
import sqlsolver.liastar.Liasolver;
import sqlsolver.liastar.Liastar;
import sqlsolver.liastar.SemiLinearSet;
import sqlsolver.uexpr.*;

import java.util.*;

import static sqlsolver.common.IterableSupport.any;
import static sqlsolver.logic.SqlSolverSupport.hasFreeTuple;
import static sqlsolver.logic.SqlSolverSupport.overlap;
...
public class SqlSolver {
  ...
  public SqlSolver(UTerm query1, UTerm query2, UVar outVar1, UVar outVar2) {
    this.query1 = query1;
    this.query2 = query2;
    this.outVar1 = outVar1;
    this.outVar2 = outVar2;
  }

  /* return EQ when q1 == q2 */
  public int proveEq() {
    if (!outVar1.equals(outVar2)) return NEQ;

    preprocess();
    UMulImpl.useWeakEquals = true;
    if (query1.equals(query2)) {
      UMulImpl.useWeakEquals = false;
      System.out.println("UExpr Equivalent");
      return EQ;
    }
    UMulImpl.useWeakEquals = false;
