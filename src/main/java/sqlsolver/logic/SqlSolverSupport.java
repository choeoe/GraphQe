package sqlsolver.logic;

import org.apache.commons.lang3.tuple.Pair;
import sqlsolver.common.NameSequence;
import sqlsolver.common.NaturalCongruence;
import sqlsolver.liastar.*;
...
import static sqlsolver.liastar.Liastar.*;
import static sqlsolver.uexpr.UExprSupport.isPredOfVarArg;
import static sqlsolver.uexpr.UExprSupport.transformSubTerms;
...
public class SqlSolverSupport {
  ...
  Liastar uexpWithSumToLiastar(UTerm t1, UTerm t2, Map<USum, String> sumVarMap,
    ...
    Liastar lia = LiaStarSimplifier.getInstance().simplify(result);
    System.out.println("Lia* simplified: ");
    ...
    switch (exp.kind()) {
      ...
      case PRED -> {
        ...
        if (pred.isPredKind(UPred.PredKind.FUNC) && isPredOfVarArg(pred)) {
          ...
          throw new UnsupportedOperationException("unsupported UPred var type: " + pred);
        }
      }
      case VAR -> {
      ...
      case PRED: {
        ...
        if (pred.isPredKind(UPred.PredKind.FUNC) && isPredOfVarArg(pred)) {
          ...
          if (curSet.contains(predVar)) {
            ...
            UPred newPred = UPred.mk(UPred.PredKind.FUNC, predName, argList, true);
...
//      rewriteSumsWithIndPred(sumList, freeTuples, existingTerms);
  ...
  private static Liastar isNullCongruence(HashMap<UTerm, String> termVarMap, HashSet<UVarTerm> isNullTuples) {
    ...
    for (UVarTerm v1 : isNullTuples) {
      ...
      isNullarg1.add(v1);
      String isNullVar1 = termVarMap.get(UPred.mk(UPred.PredKind.FUNC, NAME_IS_NULL, isNullarg1, true));
      ...
      for (UVarTerm v2 : existingIsNullTuples) {
        ...
        isNullarg2.add(v2);
        String isNullVar2 = termVarMap.get(UPred.mk(UPred.PredKind.FUNC, NAME_IS_NULL, isNullarg2, true));
    ...
    switch (u.kind()) {
      case ADD, MULTIPLY, NEGATION, SQUASH, PRED: {
        ...
        switch (u.kind()) {
          ...
          case PRED: return UPred.mk(((UPred) u).predKind(), ((UPred) u).predName(), tmp, true);
        }
      }
      case TABLE, VAR: {
...
//    Set<cypher.Pair<UVar, UVar>> eqVarPairs = collectPredicates(sums);
//    if(eqVarPairs.isEmpty()) {
//      return false;
//    }
//    for (int i = 0; i < sums.size(); ++i) {
//      UTerm cur = sums.get(i);
//      for (cypher.Pair<UVar, UVar> v : eqVarPairs) {
    ...
    switch (uexp.kind()) {
      ...
      case MULTIPLY, ADD, PRED, FUNC: {
        ...
        return switch (uexp.kind()) {
          ...
          default -> UPred.mk(((UPred) uexp).predKind(), ((UPred) uexp).predName(), newSubTerms, false);
        };
      }
      case NEGATION:
  ...
  */
  ...
  Liastar transUexpWithoutSum(boolean innerStar,
    ...
    switch (exp.kind()) {
      ...
      case CONST -> {
        Liastar result = Liastar.mkConst(false, ((UConst) exp).value());
        System.out.println(result+": "+exp);
        return result;
      }
      case TABLE -> {
        final String varName = uTermToLiaVar.computeIfAbsent((UTable) exp, t -> newUliaVarName());
        Liastar result = Liastar.mkVar(innerStar, varName);
        System.out.println(result+": "+exp);
        return result;
      }
      case PRED -> {
      ...
      case FUNC -> {
        ...
        switch (funcName) {
          ...
          case "COLLECT", "collect": {
            ...
            UTerm operand = exp.subTerms().get(0);
            Liastar liaOp = transUexpWithoutSum(innerStar, operand, uTermToLiaVar, isnullTuples, stringVars);
            return Liastar.mkFunc(innerStar, "COLLECT", liaOp);
          }
          case PredefinedFunctions.NAME_LIKE: {
            assert exp.subTerms().size() == 2;
            UTerm op0 = exp.subTerms().get(0);
          ...
          default: {
            ...
            UTerm operand = exp.subTerms().get(0);
            Liastar liaOp = transUexpWithoutSum(innerStar, operand, uTermToLiaVar, isnullTuples, stringVars);
            return Liastar.mkFunc(innerStar, funcName, liaOp);
//            throw new UnsupportedOperationException("unsupported function: " + funcName);
          }
        }
      }
